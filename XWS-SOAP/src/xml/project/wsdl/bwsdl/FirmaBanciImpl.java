...
public class FirmaBanciImpl implements FirmaBanci {
	...
	public static final String POJEDINACNA_PLACANJA_PUTANJA = "BPojedinacnaPlacanja"
			+ ID;
	public static final String TRANSAKCIJE_ZA_IZVOD = "BTransakcije" + ID; // cuvaju
																			// se
																			// obrasci
																			// mt103
	public static final String STIGLE_MT102 = "BTPristigle" + ID;
	...
	public StatusCode doClearing() {
		...
		try {
			...
			InputStream in = RESTUtil
					.retrieveResource(
							"<PojedinacnaPlacanja xmlns='http://www.project.xml/pojedinacnaplacanja'> {for $x in collection('"
									+ POJEDINACNA_PLACANJA_PUTANJA
									+ "') return $x} </PojedinacnaPlacanja>",
							POJEDINACNA_PLACANJA_PUTANJA, RequestMethod.GET);
			JAXBContext context = JAXBContext.newInstance(
					PojedinacnaPlacanja.class, PojedinacnaPlacanja.class);
			...
			PojedinacnaPlacanja rac = (PojedinacnaPlacanja) unmarshaller
					.unmarshal(reader);
			if (rac == null) {
				_return.setCode(404);
				_return.setMessage("Nema pojedinacnih placanja");
				return _return;
			}
			for (TSequence k : rac.gettSequence()) {
				System.out.println("Prolazi kroz placanja");
				String sw = cetralnaBanka.getSWIFT(k.getDuznikNalogodavac()
						.getRacun().substring(0, 3)); // kljuc za mt102
			...
			while (it.hasNext()) {
				...
				mt102.setIDPoruke("" + rnd.nextInt(1000000));
				String swiftDuznik = this.cetralnaBanka.getSWIFT(tempSeq.get(0)
						.getDuznikNalogodavac().getRacun().substring(0, 3));
				TBanke duznik = new TBanke();
				duznik.setObracunskiRacunBanke(swiftDuznik.split(",")[1]);
				duznik.setSWIFTKodBanke(swiftDuznik.split(",")[0]);
				mt102.setBankaDuznik(duznik);
				TBanke poverilac = new TBanke();
				String swiftPoverilac = this.cetralnaBanka.getSWIFT(tempSeq
						.get(0).getPrimalacPoverilac().getRacun()
						.substring(0, 3));
				poverilac.setObracunskiRacunBanke(swiftPoverilac.split(",")[1]);
				poverilac.setSWIFTKodBanke(swiftPoverilac.split(",")[0]);
				mt102.setBankaPoverilac(poverilac);
				BigDecimal ukupnaSuma = new BigDecimal(0);
				for (TSequence s : tempSeq) {
					ukupnaSuma = ukupnaSuma.add(s.getIznos());
				}
				mt102.setUkupanIznos(ukupnaSuma);

				StatusCode code = cetralnaBanka.acceptMT102(mt102);
				if (code.getCode() == 200) {
					RESTUtil.objectToDB("//" + MT102_PUTANJA,
							mt102.getIDPoruke(), mt102);
				} else {
					// ovde ako nije dobro stiglo?
					List<TSequence> seq = mt102.getSekvenca();
					for (TSequence s : seq) {
						FirmaRacun racun = findFirmu(s.getDuznikNalogodavac()
								.getRacun());
	...
	public StatusCode acceptMT910(MT910 mt910) {
		...
		try {
			...
			RESTUtil.objectToDB("//" + MT910_PUTANJA, mt910.getIDPoruke(),
					mt910);
	...
	private StatusCode uradiClearing(MT910 mt910) {
		// TODO Auto-generated method stub
		StatusCode _return = new StatusCode();
		try {
			FirmaRacun rac = findFirmu(mt910.getBankaPoverilac()
					.getObracunskiRacunBanke());
			if (rac == null) {
				_return.setCode(404);
				_return.setMessage("Bank not found");
				return _return;
			}
			MT102 mt102 = new MT102();
			mt102 = (MT102) RESTUtil.doUnmarshall("*", STIGLE_MT102 + "/"
					+ mt910.getIDPorukeNaloga(), mt102);
			if (mt102 == null
			...
			List<MT102> listaMT102 = srediSveMT102(
					procitajSveMT102(STIGLE_MT102), mt102);
			System.out.println("Sredjena lista: " + listaMT102.size());
			...
			if (listaMT102.size() == 0) {
				...
				// NIJE DOBRO OVO RESENJE - mislim da moze i malo bolje ...
				RESTUtil.createSchema(STIGLE_MT102);
			}
			rac.setRaspoloziviNovac(rac.getRaspoloziviNovac().add(
					mt910.getIznos()));
			saveRacuni();
			_return.setCode(200);
			_return.setMessage("Moo! Are you happy now? ");
			return _return;
		} catch (Exception e) {
			_return.setCode(403);
			_return.setMessage("I hear that banshee is a real screamer.");
			return _return;
		}
	}

	private List<MT102> srediSveMT102(List<MT102> list, MT102 mt102) {
		int remove = -1;
		for (int i = 0; i < list.size(); ++i) {
			if (list.get(i).getIDPoruke().equals(mt102.getIDPoruke())) {
				remove = i;
				break;
			}
		}
	...
	private List<MT102> procitajSveMT102(String putanja) {
		List<MT102> _return = new ArrayList<MT102>();
		try {
			InputStream in = RESTUtil.retrieveResource("*", putanja,
					RequestMethod.GET);
		...
		System.out.println("size mt102-ova je: " + _return.size());
		return _return;
	}

	@Override
	// not needed, not used!!!!!! ali u sustini used je veoma... majku mu...
	public StatusCode acceptMT900(MT900 mt900) {
		...
		try {
			...
			RESTUtil.objectToDB("//" + MT900_PUTANJA, mt900.getIDPoruke(),
					mt900);
	...
	private StatusCode uradiClearing(MT900 mt900) {
		StatusCode _return = new StatusCode();
		try {
			FirmaRacun racun = findFirmu(mt900.getBankaDuznik()
					.getObracunskiRacunBanke());
			if (racun == null) {
				_return.setCode(404);
				_return.setMessage("Accout not found");
				return _return;
			}
			MT102 mt102 = new MT102();
			mt102 = (MT102) RESTUtil.doUnmarshall("*", MT102_PUTANJA + "/"
					+ mt900.getIDPorukeNaloga(), mt102);
			if (mt102 == null
			...
			if (mt900.getIDPoruke().substring(2, 3).equals("e")) {
				racun.setRaspoloziviNovac(racun.getRaspoloziviNovac().add(
						mt900.getIznos()));
				_return.setCode(200);
				_return.setMessage("Returned money to acc: " + racun.getNaziv());
				saveRacuni();
				return _return;
			}
			List<MT102> listaMT102 = srediSveMT102(
			...
			if (listaMT102.size() == 0) {
				...
				// na clearing i ona koja jos nisu, uraditi u verziji 0.69
				RESTUtil.createSchema(POJEDINACNA_PLACANJA_PUTANJA);
			}
			// TO-DO
			_return.setCode(200);
			_return.setMessage("Pecemo prase kod enjaka");
		} catch (Exception e) {
	...
	public StatusCode acceptMT102(MT102 mt102) {
		...
		try {
			...
			for (TSequence sequence : mt102.getSekvenca()) {
				FirmaRacun racun = findFirmu(sequence.getPrimalacPoverilac()
						.getRacun());
				if (racun == null) {
					_return.setCode(404);
			...
			for (MT103 t : tempTransakcije) {
				RESTUtil.objectToDB("//" + TRANSAKCIJE_ZA_IZVOD, t
						.getIDPoruke().toString(), t);
			}
	...
	public StatusCode acceptMT103(MT103 mt103) {
		...
		try {
			...
			RESTUtil.objectToDB("//" + MT103_PUTANJA, mt103.getIDPoruke()
					.toString(), mt103);
			RESTUtil.objectToDB("//" + TRANSAKCIJE_ZA_IZVOD, mt103
					.getIDPoruke().toString(), mt103);
			_return.setCode(200);
			_return.setMessage("OK");
			return _return;
		} catch (Exception ex) {
	...
	public StatusCode primiNalog(NalogZaPrenos nalogZaPrenos) {
		...
		try {
			...
			FirmaRacun duznik = findFirmu(nalogZaPrenos.getDuznikNalogodavac()
					.getRacun());
			duznik.setRaspoloziviNovac(duznik.getRaspoloziviNovac().subtract(
			...
			if ((nalogZaPrenos.getIznos().doubleValue() >= 25000.00)
				...
				RESTUtil.objectToDB("//" + MT103_PUTANJA, mt103.getIDPoruke(),
						mt103);
				RESTUtil.objectToDB("//" + TRANSAKCIJE_ZA_IZVOD,
						mt103.getIDPoruke(), mt103);
				...
				if (mt900.getIDPoruke().startsWith("__")) {
					...
					duznik.setRaspoloziviNovac(duznik.getRaspoloziviNovac()
							.add(nalogZaPrenos.getIznos()));
				}
			} else {
				// Clearing & Settlement
	...
	public Presek traziIzvod(Zahtev zaDatum) {
		...
		try {
			InputStream in = RESTUtil.retrieveResource("*",
					TRANSAKCIJE_ZA_IZVOD, RequestMethod.GET);
			...
			String[] tempMT103 = xml
					.split("<MT103 xmlns=\"http://www.project.xml/MT103\" xmlns:ns2=\"http://www.project.xml/globals\">");
			System.out.println("Nasao je ovliko MT103: " + tempMT103.length);
			for (String mt103 : tempMT103) {
				...
				if (stavka_count == 0) {
					presek_count++;
					p.add(new ArrayList<Stavka>()); // p.get(0) je prazan posto
													// redni broj preseka krece
													// od 1
				}

				StringReader reader = new StringReader(
						"<MT103 xmlns=\"http://www.project.xml/MT103\" xmlns:ns2=\"http://www.project.xml/globals\">"
								+ mt103);
				...
				if (test.getDuznikNalogodavac().getRacun()
						...
						&& test.getDatumNaloga()
								.toString()
								.substring(0, 10)
								.equals(zaDatum.getDatum().toString()
										.substring(0, 10))) {
					...
					stavka.setIDPoruke("" + rnd.nextInt(100000));
					stavka_count++;
			...
			for (Stavka s : p.get(zaDatum.getRedniBrojPreseka().intValue())) {
				if (s.getRacun().getPrimalacPoverilac().getRacun()
						.equals(zaDatum.getBrojRacuna())) {
					priliv = priliv.add(s.getRacun().getIznos());
					broj_priliva++;
				}
				if (s.getRacun().getDuznikNalogodavac().getRacun()
						.equals(zaDatum.getBrojRacuna())) {
					odliv = odliv.add(s.getRacun().getIznos());
					broj_odliva++;
				}
			}
			...
			if (stanjeTemp != null) {
				zaglavlje.setPrethodnoStanje(stanjeTemp.add(odliv).subtract(
						priliv));
	...
	public void createInitial() {
		try {
			...
			RESTUtil.createSchema(POJEDINACNA_PLACANJA_PUTANJA);
			RESTUtil.createSchema(TRANSAKCIJE_ZA_IZVOD);
			RESTUtil.createSchema(STIGLE_MT102);
	...
	public void testPresek() {
		try {
			Zahtev zaDatum = new Zahtev();
			zaDatum.setBrojRacuna("101456789012345631");
			zaDatum.setRedniBrojPreseka(new BigInteger("1"));
			GregorianCalendar c = new GregorianCalendar();
			c.setTime(new Date());
			XMLGregorianCalendar date2 = DatatypeFactory.newInstance()
					.newXMLGregorianCalendar(c);
			zaDatum.setDatum(date2);
			Presek pr = this.traziIzvod(zaDatum);
			RESTUtil.createSchema("TEMP_PRESECI" + ID);
			RESTUtil.objectToDB("//TEMP_PRESECI" + ID, "", pr);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
